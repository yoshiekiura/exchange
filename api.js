...
API.init = function init(callback, allContracts, path, provider, configName, lookbackIn) {
  ...
  this.web3.version.getNetwork((error, version) => {
    ...
    // path
    if (path) {
      this.config.contractCryptex = path + this.config.contractCryptex;
      this.config.contractToken = path + this.config.contractToken;
    }

    // contracts
    this.contractCryptex = undefined;
    this.contractCryptexAddrs = [this.config.contractCryptexAddrs[0].addr];
    if (allContracts) {
      this.contractCryptexAddrs = this.config.contractCryptexAddrs.map(x => x.addr);
    }
    ...
    async.series(
      [
        (callbackSeries) => {
          this.utility.loadContract(
            this.web3,
            this.config.contractCryptex,
            this.contractCryptexAddrs[0],
            (err, contract) => {
              this.contractCryptex = contract;
      ...
      () => {
        callback(null, {
          contractCryptex: this.contractCryptex,
...
API.logs = function logs(callback, lookbackIn) {
  this.utility.blockNumber(this.web3, (err, blockNumber) => {
    ...
    async.mapSeries(
      this.contractCryptexAddrs,
      (contractCryptexAddr, callbackMap) => {
        const blocks = Object.values(this.eventsCache)
          .filter(x => x.address === contractCryptexAddr)
          .map(x => x.blockNumber);
...
API.getCryptexBalance = function getCryptexBalance(addr, callback) {
  if (addr.length === 42) {
...
API.getCryptexTokenBalances = function getCryptexTokenBalances(addr, callback) {
  if (addr.length === 42) {
    async.reduce(
      this.config.tokens,
      {},
...
API.getUSDBalance = function getUSDBalance(addr, tokenPrices, callback) {
  async.parallel(
    [
      ...
      (callbackParallel) => {
        API.getCryptexTokenBalances(addr, callbackParallel);
      },
      (callbackParallel) => {
    ...
    (err, results) => {
      const balances = { Wallet: results[0], Cryptex: results[1] };
...
API.addOrderFromEvent = function addOrderFromEvent(event, callback) {
  if (event.event === 'Order' && event.address === this.contractCryptexAddrs[0]) {
...
API.getTrades = function getTrades(callback) {
  ...
  events.forEach((event) => {
    if (event.event === 'Trade' && this.contractCryptexAddrs[0].toLowerCase() == event.address.toLowerCase()) {
...
API.getDepositsWithdrawals = function getDepositsWithdrawals(callback) {
  ...
  const events = Object.values(this.eventsCache);
  events.forEach((event) => {
    if (event.event === 'Deposit' && this.contractCryptexAddrs[0].toLowerCase() == event.address.toLowerCase()) {
      ...
      event.event === 'Withdraw' && this.contractCryptexAddrs[0].toLowerCase() == event.address.toLowerCase()
    ) {
      if (event.args.amount.toNumber() > 0) {
        const token = API.getToken(event.args.token);
        depositsWithdrawals.push({
...
API.returnTicker = function returnTicker(callback) {
  const tickers = {};
  const firstOldPrice24h  = {};
  const firstOldPrice1h   = {};
  const oldPriceFound24h  = {};
  const oldPriceFound1h   = {};
  const numberOfPrices24h = {};
  const sumOfPrices24h    = {};
  const numberOfPrices1h  = {};
  const sumOfPrices1h     = {};
  API.getTrades((err, result) => {
    ...
    trades.forEach((trade) => {
      if (trade.token && trade.base && trade.base.name === 'ETH') {
        ...
        if (!tickers[pair]) {
          tickers[pair] = { lastPrice: undefined, percentChange1h: 0, percentChange24h: 0, averagePrice24h: 0, baseVolume: 0, quoteVolume: 0 };
        }
        const tradeTime = trade.date;
        const price = Number(trade.price);
        tickers[pair].lastPrice = price;
        
        if(!oldPriceFound24h[pair]) {
          oldPriceFound24h[pair] = false;
        }
        if(!oldPriceFound1h[pair]) {
          oldPriceFound1h[pair] = false;
        }
        if(!firstOldPrice24h[pair]) {
          firstOldPrice24h[pair] = 0;
        }
        if(!firstOldPrice1h[pair]) {
          firstOldPrice1h[pair] = 0;
        }
        if(!numberOfPrices24h[pair]) {
          numberOfPrices24h[pair] = 0;
        }
        if(!sumOfPrices24h[pair]) {
          sumOfPrices24h[pair] = 0;
        }
        if(!numberOfPrices1h[pair]) {
          numberOfPrices1h[pair] = 0;
        }
        if(!sumOfPrices1h[pair]) {
          sumOfPrices1h[pair] = 0;
        }

        // 1h change
        if (Date.now() - tradeTime.getTime() < 60 * 60 * 1000) {
          // find average price
          sumOfPrices1h[pair]     += Number(price);
          numberOfPrices1h[pair]  += 1;

          if(oldPriceFound1h[pair] === false) {
            firstOldPrice1h[pair] = Number((sumOfPrices1h[pair] / numberOfPrices1h[pair]));
          }
          
          tickers[pair].percentChange1h = Number(((price - firstOldPrice1h[pair]) / firstOldPrice1h[pair] * 100).toFixed(2));
        } else {
          firstOldPrice1h[pair] = price;

          oldPriceFound1h[pair] = true;
        }
        ...
        if (Date.now() - tradeTime.getTime() < 86400 * 1000 * 1) {
          ...
          tickers[pair].lastPrice        = Number(price);

          // find average price
          sumOfPrices24h[pair]     += Number(price);
          numberOfPrices24h[pair]  += 1;
          tickers[pair].averagePrice24h  = Number((sumOfPrices24h[pair] / numberOfPrices24h[pair]));  

          if(oldPriceFound24h[pair] === false) {
            firstOldPrice24h[pair]             = tickers[pair].averagePrice24h;
          }

          tickers[pair].percentChange24h = Number(((price - firstOldPrice24h[pair]) / firstOldPrice24h[pair] * 100).toFixed(2));
        } else {
          firstOldPrice24h[pair] = price;

          oldPriceFound24h[pair] = true;
        }
      }
    });
      ...
      if (balance.lt(new BigNumber(amountGive))) {
        ...
        this.utility.sign(this.web3, addr, hash, pk, (errSign, sig) => {
          if (errSign) {
            ...
            const order = {
              contractAddr: this.contractCryptexAddrs[0],
              tokenGet,
              amountGet,
              tokenGive,
              amountGive,
